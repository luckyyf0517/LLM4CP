...
if __name__ == "__main__":
    ...
    is_U2D = 0
    prev_path = "./data/Testing Dataset/H_U_his_test.mat"  # path of dataset [H_U_his_test]
    ...
    for i in range(len(model_test_enable)):
        ...
        for speed in range(0, 10):
            ...
            if model_test_enable[i] in ['gpt', 'transformer', 'rnn', 'lstm', 'gru', 'cnn', 'np']:
                ...
                NMSE[i].append(np.nanmean(np.array(test_loss_stack)))
                SE[i].append(np.nanmean(np.array(test_loss_stack_se)) / np.nanmean(np.array(test_loss_stack_se0)))
                # BER not calculated for pad/pvec methods
                BER[i].append(0.0)
    ...
    json_path = "./outputs/results_tdd_full.json"
    ...
    plot_nmse(json_path, "./outputs/nmse_vs_velocity_tdd.png")
    save_averages(json_path, "./outputs/averages_tdd.json")
